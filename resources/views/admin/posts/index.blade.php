		...
		<div class="row mb-30">
			...
			<div class="col-lg-6">
				<div class="add-new-btn">
					<a href="{{ route('admin.post.create') }}" class="btn btn-primary float-right">{{ __('Add New') }}</a>
				</div>
			</div>
		</div>
		<div class="cart-filter">
			<a href="{{ route('admin.post.index') }}" class="btn btn-outline-primary mr-2">{{ __('All') }} <span>({{ App\Terms::where('type',0)->where('status','!=',0)->count() }})</span></a>
			<a href="?st=1" class="btn btn-outline-info mr-2">{{ __('Published') }} <span>({{ App\Terms::where('type',0)->where('status',1)->count() }})</span></a>
			<a href="?st=2" class="btn btn-outline-warning mr-2">{{ __('Drafts') }} <span>({{ App\Terms::where('type',0)->where('status',2)->count() }})</span></a>
			<a href="?st=trash" class="btn btn-outline-danger mr-2">{{ __('Trash') }} <span>({{ App\Terms::where('type',0)->where('status',0)->count() }})</span></a>
		</div>
				...
				<div class="col-lg-6">
					<form id="basicform" method="post" action="{{ route('admin.post.destroys') }}">
						...
						<div class="d-flex">
							...
							<div class="single-filter">
								<button type="submit" class="btn btn-primary ml-2 btn-lg">{{ __('Apply') }}</button>
							</div>
						</div>
					</div>
					<div class="col-lg-6">
						<div class="single-filter float-right">
							<div class="form-group">
