...
<form action="{{ route('admin.customizer.image_upload') }}" id="img_form" method="POST" enctype="multipart/form-data">
    ...
    <div class="main-sidebar-area">
        ...
        <div class="custom-form">
            ...
            @endif
            @if($settings['type'] == 'placeholder')
            <div class="form-group">
                <label for="{{ $settings['id'] }}">{{ $settings['label'] }}</label>
                <input type="text" class="form-control" id="{{ $settings['id'] }}" onchange="settings_option('{{ $settings['id'] }}','placeholder','key','{{ $option['id'] }}')" oninput="settings_option1('{{ $settings['id'] }}','placeholder','key','{{ $option['id'] }}')" value="{{ content($option['id'],$settings['id']) }}">
            </div>
            @endif
