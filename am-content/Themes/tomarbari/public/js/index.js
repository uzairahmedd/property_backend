...
featured_properties(url);
function featured_properties(url)
{
    ...
    $.ajax({
        ...
        data: {limit:9},
        beforeSend: function(){
            before_render_list('#latest_data',3);
        },
        success: function(response){ 
            ...
            if(response.data.length == 0){
               ...
               $('#latest_data').html('<div class="col-12 no-more"><h3 class="text-center">No more listing avaiable</h3></div>');

            }
            else{
                $('.show-pagination-info').show();
                $('.no-more').remove();
                 render_list('#latest_data',response.data,'mt-4');
            }
...
function basicmap()
{
        ...
        // New map
        var map = new google.maps.Map(document.getElementById('custom-map'), options);
        // Array of markers
        var markers = [];
        var asset_url = $('#base_url').val();
