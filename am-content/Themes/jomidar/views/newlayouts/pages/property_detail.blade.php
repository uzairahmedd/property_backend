...
@section('content')
        ...
        <div class="container">
            <nav aria-label="breadcrumb" class="d-flex justify-content-end align-items-center">
                <ul class="breadcrumb mb-0">
                    <li class="breadcrumb-item active theme-text-seondary-black category-title-bread"
                        aria-current="page">{{ Session::get('locale') == 'ar' ? $property->ar_title : $property->title }}</li>
                    <li class="breadcrumb-item"><a href="#"
                                                   class="theme-text-blue text-decoration-none category-breadicon"> {{ Session::get('locale') == 'ar' ? $property->property_status_type->category->ar_name : $property->property_status_type->category->name}}</a>
                    </li>
                    <li class="breadcrumb-item home-breadcrumb"><a href="/"
                                                                   class="theme-text-blue text-decoration-none first-hom-breadicon">{{__('labels.home')}}</a>
                    </li>
                </ul>
            </nav>
            <div
                class="d-flex flex-wrap-reverse justify-content-between justify-content-lg-between align-items-center my-3">
                <div class="">
                    <ul class="detail list-unstyled mb-0 d-flex flex-column flex-sm-row align-items-end justify-content-between align-items-sm-center">
                        <li class="d-flex mb-3 mb-sm-0">
                            ...
                            <div
                                class="btn btn-primary dropdown-toggle icon d-flex align-items-center justify-content-center"
                            ...
                            <ul class="dropdown-menu share-dropdown-menu" aria-labelledby="dropdownMenuButton">
                                <li><a class="dropdown-item share-drop-item"
                                       href="https://www.facebook.com/sharer.php?u={{ URL::current() }}"
                                       target="_blank"><i class="fa-brands fa-facebook-f"></i> Facebook</a></li>
                                <li><a class="dropdown-item share-drop-item"
                                       href="https://twitter.com/intent/tweet?text={{ URL::current() }}"
                                       target="_blank"><i class="fa-brands fa-twitter"></i> Twitter</a></li>
                                <li><a class="dropdown-item share-drop-item"
                                       href="https://pinterest.com/pin/create/button/?url={{ URL::current() }}&media={{ $property->post_preview->media->url ?? asset('uploads/default.png') }}"
                                       target="_blank"><i class="fa-brands fa-pinterest-p"></i> Pinterest</a></li>
                                <li><a class="dropdown-item share-drop-item"
                                       href="https://api.whatsapp.com/send?text={{ URL::current() }}" target="_blank"><i
                                            class="fa-brands fa-whatsapp"></i> Whatsapp</a></li>
                                <li><a class="dropdown-item share-drop-item"
                                       href="mailto:{{ $property->user->email }}?subject={{ $property->title }}&body={{ URL::current() }}"><i
                                            class="fa-regular fa-envelope"></i> Gmail</a></li>
                            </ul>
                        </li>
                        <li class="d-flex mb-3 mb-sm-0">
                            ...
                            @if (Auth::check())
                                ...
                                @endphp
                                <a href="javascript:void(0)" onclick="favourite_property('{{ $property->id }}')"
                                   id="favourite_btn" class="{{ $property_id == $property->id ? 'active' : '' }}">
                                    <i class="{{isset($property_id) ? 'fa-solid' : 'fa-regular'}} fa-heart ps-1"
                                       id="heart"></i>
                                </a>
                            @else
                ...
                <div class="col-12 col-lg-6">
                    ...
                    <div class="basic-info-detail">
                        ...
                        @foreach ($property_type_nature as $value)
                            @if( $value->type == 'parent_category')
                                <div class="row w-100 mb-3">
                                    ...
                                    <span
                                        class="font-16 theme-text-seondary-black b-info-txt">{{__('labels.property_nature')}}</span>
                                    </div>
                                </div>
                            @elseif( $value->type == 'category')
                                <div class="row w-100 mb-3">
                                    ...
                                    <div class="col-6 detail-txt-left">
                                    <span
                                        class="font-16 theme-text-seondary-black b-info-txt">{{__('labels.type_property')}} </span>
                                    </div>
                                </div>
                            @endif
                        @endforeach
                        ...
                        <div class="row w-100 mb-3">
                            ...
                            <div class="col-6 detail-txt-left">
                            <span
                                class="font-16 theme-text-seondary-black b-info-txt">{{__('labels.electricity_meter_is_there')}}</span>
                            </div>
                        </div>
                        ...
                        <div class="row w-100 mb-3">
                            ...
                            <div class="col-6 detail-txt-left">
                            <span
                                class="font-16 theme-text-seondary-black b-info-txt">{{__('labels.water_meter_is_there')}}</span>
                            </div>
                        </div>
                    ...
                    @if(count($property->option_data) > 0 || !empty($property->property_condition) || !empty($property->role))
                        ...
                        @foreach ($property->option_data as $options_data)
                            <div class="row w-100 mb-3">
                                ...
                                <div class="col-6 detail-txt-left">
                                    <span
                                        class="font-16 theme-text-seondary-black"> {{ Session::get('locale') == 'ar' ? $options_data->category->ar_name : $options_data->category->name}}</span>
                                </div>
                            </div>
                        @endforeach
    ...
    <!--Send Message Modal -->
    <div class="modal fade theme-modal send-modal" id="inquiry_form" tabindex="-1" aria-labelledby="exampleModalLabel"
         aria-hidden="true">
                            ...
                            <form action="{{ route('property.inquiryform') }}" method="POST" id="inquiryform">
                                ...
                                <div class="mb-4_5 position-relative">
                                    <input type="text" placeholder="{{ __('Your Name') }}" name="name"
                                           class="form-control font-medium font-16">
                                    <label for="floating-Input"
                                           class="floating-Input position-absolute font-medium theme-text-seondary-black b-r-8">اسمك</label>
                                </div>
                                <div class="position-relative mb-4_5">
                                    <input type="email" placeholder="{{ __('Email') }}" name="email"
                                           class="form-control font-medium font-16">
                                    <label for="floating-Input"
                                           class="floating-Input position-absolute font-medium theme-text-seondary-black b-r-8">البريد
                                        الإلكتروني</label>
                                </div>
                                <div class="position-relative mb-4_5">
                                    <textarea name="message" cols="30" rows="5" class="form-control"
                                              placeholder="{{ __('Message') }}" class="form-control font-medium font-16"
                                              style="height: 100px;"></textarea>
                                    <label for="floating-Input"
                                           class="floating-Input position-absolute font-medium theme-text-seondary-black b-r-8">رسالة</label>
                                </div>
                                <input type="hidden"
                                       value="{{ isset($property->user->email) ? $property->user->email : 'N/A'  }}"
                                       name="agent_email">
                                ...
                                @endif
                                <button type="submit"
                                        class="basicbtn chat-btn theme-bg-sky theme-text-white border-0 font-bold font-16">
                                    {{ __('Send Message') }}
                                </button>
                            </form>
                        </div>
...
@push('js')
    ...
    <script
        src="https://maps.googleapis.com/maps/api/js?key={{ env('GOOGLE_API_KEY') }}&callback=initialize&libraries=&v=weekly"
        defer></script>
@endpush
