...
var last_id1 = $('.last_id1').val();
    			...
    			$.ajax({
    				...
    				success: function(response){ 
    					...
    					$.each(data, function(index, value){
    						$(".popupmedia").append("<div class='col-lg-2 media-hover media-item'><input type='hidden' id='single_media"+value.id+"' value='"+value.url+"'><label class='single-img'  for='me"+value.url+"' id='media-single-level"+value.id+"' onclick='single_active("+value.id+")'><div class='single-media model-media text-center'><img class='img-fluid' src='"+small(value.url)+"' ></div></label></div>");
    						
    					});
    					
    					$('.last_id1').val(response.next_page_url);
    		...
    		if ($reload==true) {
    			$.ajax({
    				...
    				success: function(response){ 
    					...
    					$('.multi_last').val(response.next_page_url);
    					if (response.next_page_url == null) {
	...
	function single_active(id) {
		...
		var my_img= $('#single_media'+id).val();
		
		myradiovalue=my_img;
		$('#previewimg').attr('src',my_img);
		$('#medialink').val(my_img)
