...
<div class="row">
	<div class="col-lg-9">
		<div class="card">
			<div class="card-body">
				<h4>{{ __('Edit Agency') }}</h4>
				<form method="post" action="{{ route('admin.agency.update',$agency->id) }}" id="basicform">
					@csrf
					<div class="pt-20">
                        @php
                        ...
                        <div class="form-group">
                            ...
                            <select class="custom-select mr-sm-2 select2" id="user" name="user_id[]" multiple>
                                @foreach ($users as $value)
                                    <option @foreach($agency->agency as $data) {{ $data->user_id == $value->id ? 'selected' : '' }} @endforeach value="{{ $value->id }}">{{ $value->email }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
						...
						$arr['id']= 'email';
						$arr['type']= 'email';
						$arr['placeholder']= 'Enter Email Address';
						$arr['name']= 'email';
                        $arr['is_required'] = false;
